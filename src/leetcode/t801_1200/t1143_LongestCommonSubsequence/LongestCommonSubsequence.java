package leetcode.t801_1200.t1143_LongestCommonSubsequence;
...
//                    System.out.println(String.format("dp[%d][%d] = dp[%d][%d] + 1 = %d + 1;", (i+1), (j+1), i, j, dp[i][j]));
                    dp[i + 1][j + 1] = dp[i][j] + 1;
                } else {
//                    System.out.println(String.format("dp[%d][%d] = Math.max(dp[%d][%d], dp[%d][%d])\n= Math.max(%d, %d);", (i+1), (j+1), (i+1), j, i, (j+1), dp[i + 1][j], dp[i][j + 1]));
